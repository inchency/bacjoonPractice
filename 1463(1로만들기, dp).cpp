...
int dp(vector<int>dpTable, int num) {
	...
	for (int i = 4; i <= num; i++) {
		...
		int min;
		if ((i % 2 == 0) && (i % 3 == 0)) { // 둘다 나눠지면
			temp2 = i / 2;
			temp3 = i / 3;
			temp4 = i - 1;
			min = dpTable[temp2];  // 최소값 찾기위해
			...
			temp2 = i / 2;
			temp4 = i - 1;
			min = dpTable[temp2]; // 최소값 찾기 위해
			if (min > dpTable[temp4]) {
				min = dpTable[temp4];
			}
		}
		else if (i % 3 == 0) { // 3으로 나눠지면
			temp3 = i / 3;
			temp4 = i - 1;
			min = dpTable[temp3]; // 최소값 찾기 위해
			if (min > dpTable[temp4]) {
				min = dpTable[temp4];
			}
		}
		else if ((i % 2 != 0) && (i % 3 != 0)) { // 2와 3으로 안눠지면 -1
			min = dpTable[i - 1];
		}
		dpTable[i] = 1 + min;
	}
